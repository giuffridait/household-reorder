...
export default function Home() {
  ...
  const buyNow = activeItems.filter(
    (i) => i.status === 'active' && i.recommendation.urgency === 'buy_now'
  );
  const buyThisWeek = activeItems.filter(
    (i) => i.status === 'active' && i.recommendation.urgency === 'buy_this_week'
  );
  const watch = activeItems.filter(
    (i) => i.status === 'active' && i.recommendation.urgency === 'watch'
  );
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-start justify-between mb-8">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-gray-900 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-gray-700 transition-colors"
          >
      ...
      {showAddModal && (
        <ItemFormModal onClose={() => setShowAddModal(false)} onSave={handleSaveForm} />
      )}
      {editingItem && (
        <ItemFormModal
          item={editingItem}
      ...
      {feedbackItem && (
        <FeedbackModal
          ...
          onSubmit={(feedback) => doMarkOrdered(feedbackItem.id, feedback)}
        />
      )}
    </main>
  );
...
function Section({
  ...
  return (
    <section>
      ...
      {items.length === 0 ? (
        emptyMessage ? (
          <p className="text-sm text-gray-400 pl-4">{emptyMessage}</p>
        ) : null
      ) : (
  ...
  return (
    <div className="text-center py-24">
      ...
      <p className="text-sm text-gray-400 mb-6">
        Add items to never run out of household essentials.
      </p>
      <button
        onClick={onSeed}
        disabled={seeding}
        className="text-sm text-gray-500 border border-gray-200 px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors disabled:opacity-50"
      >
